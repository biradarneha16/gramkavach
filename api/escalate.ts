...
export default async function handler(request: any, response: any) {
  ...
  if (!accountSid || !authToken || !fromNumber) {
    response.status(200).json({
      note: "Twilio environment variables are missing. Cyber crime escalation stayed in simulation mode."
    });
    return;
  }
  ...
  try {
    ...
    response.status(200).json({
      note: `Escalation accepted by Twilio with SID ${result.sid}.`
    });
  } catch {
    response.status(502).json({ note: "SMS provider rejected the escalation." });
  }
}
